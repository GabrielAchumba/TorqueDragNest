...
export class TrippingOut {
    ...
    casingFF:number = null, openHoleFF:number = null):OperationResult[]
    {
        ...
        for (ii = 0; ii < pipesLength; ii++)
        {
            ...
            let operationResult = {...OperationResultObj} as OperationResult;
            ...
            if (ii == 0)
            {
            operationResult.tensionBottomOfPipe = 0;
            operationResult.torqueBottom = 0;
            ...
            operationResult.normalForce = Drag.NormalForce(operationResult.tensionBottomOfPipe, operationResult.averageInclination,
            ...
            Math.cos(operationResult.averageInclination)
            ...
            frictionFactor,
            operation.trippingOutSpeed, operation.trippingOutRPM, operationResult.pipeOuterDiameter, 0);
            ...
            * operationResult.pipeOuterDiameter)) * Math.atan(k * L) / (k * L);
            ...
            operationResult.torequeFromHelical = 0;// tensionTopOfPipe * Math.Pow(operationResult.radialClearance, 2) * beta / term3;
            operationResult.shearStressFromHelical = 0;// tensionTopOfPipe * operationResult.radialClearance * beta / term3;
        ...
        let HKL:number = 0, dHKL:number = 0;
        const blockWeight:number = common.blockWeight * M;
        const ncount:number = operationResults.length;
        for (ii = ncount - 1; ii >= 0; ii--)
        {
            dHKL = Math.abs(operationResults[ii].buoyancyWeight * Math.cos(operationResults[ii].averageInclination)) + Fds[ii];
            HKL = HKL + dHKL;
