export class FrictionalPressurePowerLaw
{
    public static  PressureLaminarDrillBit(mudDensity:number, effectiveVelocity:number)
    {
        const deltaP:number = mudDensity * Math.pow(effectiveVelocity, 2) / 1120;
        return deltaP;
    }

    public static PressureLaminarInsidePipe(consistencyIndex:number, flowBehaviourIndex:number,
                            effectiveVelocity:number, pipeInnerDiameter:number,
    ...
    public static PressureLaminarInsideAnnulus(consistencyIndex:number, flowBehaviourIndex:number,
